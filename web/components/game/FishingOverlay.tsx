...
export default function FishingOverlay() {
  ...
  const spotRef = useRef<{ x: number; z: number; zone?: "river" | "sea" } | null>(null);
  ...
  const hook = () => {
    ...
    const zone: "river" | "sea" = sp?.zone ?? (sp && coastDist(sp.x, sp.z) > 47 ? "sea" : "river");
  ...
  useEffect(() => {
    const onStart = (e: Event) => {
      const d = (e as CustomEvent<{ x: number; z: number; zone?: "river" | "sea" }>).detail;
      if (d && typeof d.x === "number") spotRef.current = { x: d.x, z: d.z, zone: d.zone };
