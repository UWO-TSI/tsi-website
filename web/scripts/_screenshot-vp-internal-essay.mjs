...
const testEmail = `int-shot-${Date.now()}@tethos-test.dev`;
...
try {
  await page.goto(`${APP_URL}/student/apply/vp-internal`, { waitUntil: "networkidle" });
  ...
  await page.click('button:has-text("Next")');
  await page.waitForSelector("text=Cottage Trip", { timeout: 8000 });
  await page.waitForTimeout(1000);

  await page.screenshot({ path: "/tmp/vp-internal-essay-step.png", fullPage: true });
  console.log("✓ /tmp/vp-internal-essay-step.png");
} finally {
  await browser.close();
