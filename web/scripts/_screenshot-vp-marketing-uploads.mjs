...
const testEmail = `mkt-twoshot-${Date.now()}@tethos-test.dev`;
...
try {
  ...
  // Capture resume step (resume + portfolio uploads side-by-side)
  await page.screenshot({ path: "/tmp/vp-mkt-resume-step.png", fullPage: true });
  console.log("✓ /tmp/vp-mkt-resume-step.png");

  // Upload resume to advance
  ...
  await page.screenshot({ path: "/tmp/vp-mkt-essay-step.png", fullPage: true });
  console.log("✓ /tmp/vp-mkt-essay-step.png");
} finally {
  await browser.close();
