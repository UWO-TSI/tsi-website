...
export default function EditEmotePage({
  params,
}: {
  ...
  const [row, setRow] = useState<EmoteType | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (error || !data) {
          setError(error?.message ?? "Emote not found");
        } else {
          setRow(data as EmoteType);
        }
      } catch (err) {
        if (!cancelled) {
          setError(err instanceof Error ? err.message : "Failed to load emote");
        }
      } finally {
  ...
  return <EmoteEditor mode="edit" rowId={id} initial={row} />;
}
