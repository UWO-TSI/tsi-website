...
- **Project tabs (PR #39):** after Marketing, one tab per developer project (BGC London, ArkAid, Grand Theatre, Brain Tumour, Growing Chefs) listing every developer who ranked it in their top 3, full application with "Their rank for this project" and "Why these picks" up front. Fixed rows per (project, applicant) in `recruitment_sheet_project_rows` (migration `20260918230000`), assigned at queue time from the picks; picks never change after submission. Developers who ranked nothing appear only in the Developers tab.
- Vercel, twice on 2026-09-18: the #34 merge got no production deployment at all, and the #36 deployment built READY but was never promoted to the domain (www.tethos.ca stayed on the #35 deployment). Each time a fresh merge fixed it. The duplicate `uwotsi` project builds every push too and shares the hobby plan's single build slot; deleting it is overdue.
